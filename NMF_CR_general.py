...
def DecomAdjMat(adj_mat, weight_mat, alpha_0, alpha_1, beta_0, lambda_0, lambda_1, max_iter_num, k, dis_val):
    ...
    while i <= max_iter_num:
        ...
        U_lr_numerator = alpha_0 * W2_had_A_V + alpha_1 * A_A_tran_U_C_tran_add_C
        ...
        U_lr_denominator = alpha_0 * W2_had_U_V_tran_V + alpha_1 * (U_C_U_tran_U_C_tran + U_C_tran_U_tran_U_C) + lambda_0 * U
        U_lr_denominator = np.maximum(U_lr_denominator, np.where(U_lr_denominator < 1.0e-16, 1, 0) * dis_val)
        ...
        V_lr_numerator = alpha_0 * W2_had_A_tran_U
        ...
        V_lr_denominator = alpha_0 * W2_had_U_V_tran_update_tran_U + lambda_1 * V
        V_lr_denominator = np.maximum(V_lr_denominator, np.where(V_lr_denominator < 1.0e-16, 1, 0) * dis_val)
        ...
        C_lr_numerator = alpha_1 * U_tran_A_A_tran_U + beta_0 * C.transpose()

        U_tran_U = np.matmul(U.transpose(), U)
        U_tran_U_C = np.matmul(U_tran_U, C)
        U_tran_U_C_U_tran_U = np.matmul(U_tran_U_C, U_tran_U)

        C_lr_denominator = alpha_1 * U_tran_U_C_U_tran_U + beta_0 * C
        C_lr_denominator = np.maximum(C_lr_denominator, np.where(C_lr_denominator < 1.0e-16, 1, 0) * dis_val)
        ...
        L = alpha_0 / 2 * L_A + alpha_1 / 2 * L_A_A_tran +  beta_0 / 4 * L_C_C_tran  + lambda_0 / 2 * L_U + lambda_1 / 2 * L_V
